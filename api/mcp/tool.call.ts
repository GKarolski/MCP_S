...
type T = { base:string; key:string; secret:string };
...
// próba wydobycia numeru śledzenia z popularnych meta
function extractTracking(meta:any[]){
  if(!Array.isArray(meta)) return null;
  const byKey = (k:string) => meta.find((m:any)=>m?.key===k)?.value;
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try{
    ...
    const shippingLines = (o?.shipping_lines||[]).map((s:any)=>({
    ...
    const result = {
      ...
      customer: {
        email: o.billing?.email, name: `${o.billing?.first_name||""} ${o.billing?.last_name||""}`.trim()
      },
      addresses: { billing: o.billing, shipping: o.shipping },
      items,
      shipping_lines: shippingLines,
      tracking, // null jeśli brak w metadanych / brak wtyczki tracking
      raw_meta: o.meta_data // dla debug, usuń w prod jeżeli niepotrzebne
    };
